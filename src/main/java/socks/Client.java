...
public class Client
{
    ...
    void Receive_Response()
    {
        ...
        try
        {
            ...
            if (_class.equals("MessageListResponse"))
            {
                ...
                {
                    System.out.println("[TIME] IDENTITY: MESSAGE".replaceAll("TIME", new SimpleDateFormat("HH:mm").format(message.when)).replaceAll("IDENTITY", message.from).replaceAll("MESSAGE", message.body));
                }
            }
        }
        catch (IOException error)
    ...
    void Get()
    {
        ...
        try
        {
            get_request.after = new SimpleDateFormat("dd/MM/yyyy HH:mm").parse(scanner.nextLine()).getTime();
        }
