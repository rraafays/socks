...
public class Client {
  ...
  void Receive_Response() {
    ...
    try {
      ...
      if (_class.equals("ErrorResponse")) {
        System.out.println(
            "\u001B[31m" + mapper.readValue(json, Error_Response.class).error + "\u001B[0m");
      }
      if (_class.equals("MessageListResponse")) {
        Message_List_Response message_list_response =
            mapper.readValue(json, Message_List_Response.class);
  ...
  void Get() {
    ...
    System.out.println(
        "\u001B[33mAfter what date would you like messages from? E.G: 01/01/2023 10:30\u001B[0m");
    try {
      get_request.after =
          new SimpleDateFormat("dd/MM/yyyy HH:mm").parse(scanner.nextLine()).getTime();
